...
function Header(){
  ...
  return <header className="App-header">
    ...
    <ButtonGroup style={{marginLeft:5, display:'flex'}}>
        <Button style={{height:'3rem'}} type={mode==='hourly'?'primary':'default'}
        onClick={()=>ctx.set({mode:'hourly'})}>
          Hourly
        </Button>
        <Button style={{height:'3rem'}} type={mode==='daily'?'primary':'default'}
        onClick={()=>ctx.set({mode:'daily'})}>
          Daily
        </Button>
    </ButtonGroup>
  </header>
